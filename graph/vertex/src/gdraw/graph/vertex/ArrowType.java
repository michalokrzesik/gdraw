...
public enum ArrowType implements Serializable {
    None {
        public void draw(GraphicsContext gc, Paint color, VertexPoint source, VertexPoint destination, VertexType type) {
    ...
    Opened {
        @Override
        public void draw(GraphicsContext gc, Paint color, VertexPoint source, VertexPoint destination, VertexType type) {
            double[] points = arrowPoints(source, destination, gc.getLineWidth() + 7, type);
            gc.setLineDashes(null);
...
//            arrows.add(polygon);
    ...
    public static double[] arrowPoints(VertexPoint from, VertexPoint destination, double arrowHeadSize, VertexType type) {
        VertexPoint source = type == VertexType.Straight ? from : new VertexPoint(VertexType.controlPoint(from, destination));
    ...
    abstract public void draw(GraphicsContext gc, Paint color, VertexPoint source, VertexPoint destination, VertexType type);
}
